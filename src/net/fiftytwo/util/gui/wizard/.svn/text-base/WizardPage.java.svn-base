 ...
 * WizardPage.java
 *
 * Created on September 30, 2002, 11:24 AM
 */
...
/**
 * Extend this class to create a page for the Wizard.  Implement the
...
{
    /**
     *  This code was borrowed (stolen) from the JAS site http://www-sldnt.slac.stanford.edu/jas/
     *
     ...
     */
    public WizardPage (final java.awt.LayoutManager lm)
    {
	super (lm);
	
	m_isFinishable = (this instanceof WizardIsFinishable);
	m_hasNextPages = (this instanceof WizardHasNextPages);
    }
    ...
    public void setLayoutManager(final java.awt.LayoutManager lm)
    {
	this.setLayout(lm);
    }
    final WizardPage getPrev ()
    {
	return m_prev;
    }
    final void addTo (final Container c, final Wizard wizard, final WizardPage prev)
    {
	c.add (m_number = String.valueOf (pageNumber++), this);
	if (m_hasNextPages)
	{
	    m_nextWizardPages = ((WizardHasNextPages) this).getNextWizardPages ();
     ...
     *  @see WizardPage#getNextEnabled()
     *  @see WizardPage#getFinishEnabled()
     *  @see WizardPage#keyReleased(KeyEvent)
    ...
    {}
     ...
     public void setWizard(Wizard wizParent)
    {
    }
